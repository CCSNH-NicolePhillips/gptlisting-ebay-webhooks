...
import { ensureInventoryItem, createOffer, publishOffer, ensureEbayPrereqs } from '../services/ebay.js';
...
processRouter.post('/process', async (req, res) => {
  try {
    const body = BodySchema.parse({ ...req.body, limit: req.query.limit ? Number(req.query.limit) : undefined });
    ...
    if (!cfg.ebay.policy.paymentPolicyId || !cfg.ebay.policy.returnPolicyId || !cfg.ebay.policy.fulfillmentPolicyId || !cfg.ebay.merchantLocationKey) {
    ...
    for (const g of groups) {
      ...
      try {
        ...
        } catch (e) { /* ignore parse errors and fallback to defaults */ }
        ...
        results.push({ sku: g.sku, offerId: offer.offerId, mode: body.mode || 'draft', price: ebayPrice, floor });
      } catch (err:any) {
