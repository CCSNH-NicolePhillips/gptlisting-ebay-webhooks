...
async function analyzeBatchViaVision(
  ...
  const prompt = [
    ...
    "",
    "Step 3 — PRODUCT FIELDS & BRAND DETECTION (CRITICAL):",
    "Now that you have the role (front, back, side, other) and the extracted text, fill in the product fields.",
    "You MUST follow this exact order of operations for the brand field:",
    "",
    "3.1 Fields to Extract:",
    "• brand (company or product line name)",
    "• product (short name of the product) — see 3.1a below for STRICT rules",
    "• variant / flavor (if any)",
    "• size / servings",
    "• category and categoryPath (parent > child)",
    "• options object (Flavor, Formulation, Features, Ingredients, Dietary Feature)",
    "• claims[] (marketing claims printed on the label)",
    "• confidence in the overall identification (0.0–1.0)",
    "",
    "3.1a Product Name Detection – STRICT (CRITICAL):",
    "• The product field MUST be the exact product name text as actually printed on the packaging — a real wordmark/title, verbatim (preserve capitalization/wording).",
    "• The product name is usually the SECOND most prominent text on the panel, right below or near the brand logo (e.g., brand='TRULY', product='Smooth Legend').",
    "• NEVER invent, paraphrase, summarize, or generate a marketing-style description as the product name.",
    "  - Do NOT synthesize a name from the ingredient list or claims (e.g., do NOT write 'Moisture-Drenching Serum' just because the label lists hydrating ingredients like squalane or plum oil).",
    "  - Do NOT combine the product type with an invented adjective (e.g., 'Smoothing Serum', 'Hydra-Boost Cream') unless that exact phrase is printed on the packaging.",
    "• If a product type/category is printed (e.g., 'AFTER SHAVE SERUM', 'BODY LOTION') that is a separate field — it belongs in category, NOT in product, unless no other product name text exists at all.",
    "• If you cannot find a distinct printed product name separate from the brand and category, it is BETTER to return product=\"\" with low confidence than to invent a plausible-sounding one.",
    "",
    "3.2 Brand Detection – General Principles:",
    "• A brand is a company or product line name, NOT an ingredient, vitamin, sweetener, or chemical.",
    "• When you are uncertain, it is BETTER to return an EMPTY brand (\"\") with low confidence than to guess incorrectly.",
    "• Strong indicators that a word is NOT a brand:",
    "  - Appears inside or immediately under a Supplement Facts / Nutrition Facts / Ingredients section",
    "  - Is followed by amounts like mg, mcg, g, %, IU",
    "  - Looks like a single chemical/ingredient name (e.g., Stevia, Biotin, Magnesium, Zinc, Collagen, Ashwagandha, Vitamin C, Hyaluronic Acid)",
    "• NEVER choose these words as a brand for supplements:",
    "  'Stevia, Biotin, Magnesium, Zinc, Vitamin C, Vitamin D, Collagen, Hyaluronic Acid, Ashwagandha, Creatine, BCAA, Electrolytes, Caffeine, Melatonin, Probiotic, Prebiotic, Inositol, VitaCholine, Glucosamine, Chondroitin, Turmeric, Ginger, CoQ10, Omega-3'",
    "  (This list is illustrative; treat any similar ingredient word the same way.)",
    "",
    "3.3 Brand Detection for FRONT Panels (role = front):",
    "• Primary source: The largest, most prominent logo/wordmark on the front of the packaging.",
    "• Prefer: A short phrase or 1–3 words near the top or center of the front.",
    "• Words that look like a company or brand (e.g., 'Jocko Fuel', 'RKMD', 'Root', 'Naked Nutrition', 'RYSE').",
    "• AVOID: Pure flavor names ('Peach Mango', 'Strawberry', 'Vanilla'), generic product types ('Greens Powder', 'Pre-Workout', 'Collagen Peptides').",
    "• If there are multiple candidates, choose the one that most clearly looks like a brand/company rather than the product type.",
    "• If still unclear, set brand=\"\" and confidence ≤ 0.5.",
    "",
    "3.4 Brand Detection for BACK Panels (role = back):",
    "You MUST follow these steps IN ORDER:",
    "",
    "STEP A — Find the Manufacturer / Distributor Block:",
    "• Search the extracted text for lines containing any of: 'Manufactured by', 'Manufactured for', 'Distributed by', 'Marketed by', 'Company', 'LLC', 'Inc', 'Labs', 'Corporation', 'Corp'.",
    "• If found, treat the one or two lines immediately following or surrounding that phrase as the manufacturer block.",
    "• Inside that block, identify the most likely brand:",
    "  - A company-like name before the address",
    "  - Often 1–4 words, sometimes in ALL CAPS or Title Case",
    "  - Example: from 'Manufactured for RKMD, Austin, TX 78701' → brand = 'RKMD'.",
    "• IMPORTANT: Save this as candidateBrandFromManufacturer (do NOT finalize yet - continue to Step B)",
    "",
    "STEP B — Look for Visible Logo / Brand Wordmark on Back Panel:",
    "• BEFORE using the manufacturer text, check if there is a visible brand logo or brand name on the back panel itself:",
    "  - Look for prominent brand text/logo (separate from the manufacturer block)",
    "  - Check for a brand name repeated from what might be on the front",
    "  - Look near the top, center, or as a watermark",
    "  - Example: Small 'RKMD' logo in corner, or 'RKMD' text near top",
    "• If a clear brand logo/wordmark is found:",
    "  - Use THIS as the brand (prioritize visual brand over manufacturer text)",
    "  - Manufacturer may be a contract manufacturer, not the actual brand",
    "  - Example: Logo says 'RKMD' but manufacturer says 'Vitaminne' → brand = 'RKMD'",
    "• If NO visible logo found:",
    "  - Use candidateBrandFromManufacturer from Step A",
    "",
    "STEP C — Ingredient Guard (MUST ALWAYS RUN):",
    "• Once you have a candidate brand, you MUST validate that it is NOT just an ingredient:",
    "  1. Extract all text from: 'Supplement Facts', 'Nutrition Facts', 'Drug Facts', 'Ingredients', 'Other Ingredients', 'Inactive Ingredients'",
    "  2. Build a list of ingredient-like tokens from those sections",
    "  3. If the candidate brand:",
    "     - Appears inside this ingredient list, OR",
    "     - Is immediately followed by 'mg', 'mcg', 'g', '%', 'IU', or a dosage amount, OR",
    "     - Looks like a plant, vitamin, sweetener, or chemical name,",
    "     → THEN IT IS NOT A VALID BRAND.",
    "  4. In that case:",
    "     - DO NOT use it as the brand",
    "     - Set brand=\"\" and confidence ≤ 0.4",
    "",
    "CRITICAL EXAMPLE:",
    "• If the back panel contains 'Stevia' only in the ingredient list as a sweetener and the manufacturer text says 'Manufactured by RKMD', then:",
    "  - brand = 'RKMD' (CORRECT)",
    "  - NEVER brand = 'Stevia' (Stevia must be treated only as an ingredient)",
    "• If the back panel shows 'Distributed by: Vitaminne, LLC' but has an 'RKMD' logo visible:",
    "  - brand = 'RKMD' (CORRECT - use the logo, not the distributor)",
    "  - Vitaminne is just the distributor/contract manufacturer, NOT the brand",
    "",
    "STEP D — No Brand Found:",
    "• If, after steps A–C:",
    "  - No valid manufacturer name is found,",
    "  - No clear brand/logo is visible, or",
    "  - Every candidate fails the ingredient guard,",
    "• Then: Set brand = \"\" and confidence ≤ 0.4",
    "• DO NOT GUESS a brand.",
    "",
    "3.5 Non-Product or Irrelevant Images:",
    "• For non-product or unbranded images (e.g., random objects, scenery, furniture):",
    "  - Set brand = 'Unknown'",
    "  - Set product = 'Unidentified Item'",
    "  - Set confidence ≤ 0.3",
    "",
    "3.6 Final Consistency Check:",
    "• Before returning JSON, ensure the final brand:",
    "  - Does NOT belong to any ingredient section",
    "  - Does NOT look like a vitamin, plant, sweetener, or nutrient",
    "  - Is consistent with the role (front/back) and packaging style",
    "• If you cannot satisfy these checks with high confidence:",
    "  - PREFER brand=\"\" with low confidence over a wrong brand",
    "",
    ...
    "• The product field MUST be verbatim text actually printed on the packaging, NEVER a description you generated from ingredients/claims — re-check this before returning JSON",
  ].join("\n");
