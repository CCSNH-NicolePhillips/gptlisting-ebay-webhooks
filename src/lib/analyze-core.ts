...
async function analyzeBatchViaVision(
  ...
  const prompt = [
    ...
    "For EVERY image also include quick insights: hasVisibleText (true/false), dominantColor (one of: black, white, gray, red, orange, yellow, green, blue, purple, brown, multi), role (front, back, side, detail, accessory, packaging, other), and textExtracted (ALL visible text you can read on the image - be VERY thorough, especially for product backs which often have lots of text).",
