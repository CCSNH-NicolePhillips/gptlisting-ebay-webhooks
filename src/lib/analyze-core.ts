...
async function analyzeBatchViaVision(
  ...
  const prompt = [
    ...
    "CRITICAL FOR visualDescription: Be extremely detailed and specific. Instead of 'green bottle', say 'Forest green cylindrical pouch with silver/white text, vertical text layout, supplement facts panel visible'. Include: packaging type (pouch/bottle/box/jar/tube), shape (cylindrical/rectangular/oval), primary colors (specific shades), text colors, text layout (vertical/horizontal/diagonal), any visible panels (nutrition/ingredients/barcode), prominent design elements.",
