...
export function tokensStore() {
  const siteID = process.env.NETLIFY_BLOBS_SITE_ID || process.env.BLOBS_SITE_ID || undefined;
  const token = process.env.NETLIFY_BLOBS_TOKEN || process.env.BLOBS_TOKEN || undefined;
  if (siteID && token) {
