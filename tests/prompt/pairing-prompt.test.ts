...
describe('pairing prompts', () => {
  it('returns the exact system prompt text', () => {
    expect(getPairingSystemPrompt()).toBe(expectedSystemPrompt);
  });

  it('returns the exact user prompt text', () => {
    expect(getPairingUserPrompt()).toBe(expectedUserPrompt);
  });
});
