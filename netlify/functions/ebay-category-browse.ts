...
export const handler: Handler = async (event) => {
	try {
		...
		function mapChildren(
			node: any,
			ancestors: any[] = []
		): { node: BrowseNode; children: BrowseNode[]; breadcrumbs: BrowseNode[] } {
			...
			const kids = (cat?.childCategoryTreeNodes || node.childCategoryTreeNodes || []).map(
				(c: any) => {
					...
					return {
						id: cId,
						name: cName,
						path: [...ancestors.map((a) => a.name), name, cName].join(' > '),
						leaf,
					} as BrowseNode;
				}
			);
			const here: BrowseNode = {
				id,
				name,
				path: [...ancestors.map((a) => a.name), name].join(' > '),
				leaf: kids.length === 0,
			};
		...
		if (!categoryId) {
			...
			return {
				...
				body: JSON.stringify({ ok: true, treeId, ...res }),
			};
		}
		...
		return {
			statusCode: 500,
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ error: 'category-browse error', detail: e?.message || String(e) }),
		};
	}
};