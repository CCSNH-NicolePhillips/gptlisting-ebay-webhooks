...
export const handler: Handler = async (event) => {
	try {
		...
		const name =
			(qs['name'] || process.env.SHIP_NAME || '').toString().trim() || 'Default Location';
		...
		if (!addressLine1 || !city || !stateOrProvince || !postalCode || !country) {
			return {
				...
				body: JSON.stringify({
					...
					example:
						'/.netlify/functions/ebay-create-location?key=home&name=Home&address1=123%20Main&city=Manchester&state=NH&postal=03101&country=US',
				}),
			};
		}
		...
		return {
			statusCode: 500,
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ error: 'create-location error', detail: e?.message || String(e) }),
		};
	}
};