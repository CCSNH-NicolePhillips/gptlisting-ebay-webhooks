...
export const handler: Handler = async (event) => {
  try {
    ...
    const url = `${apiHost}/sell/account/v1/privilege`;
    ...
    return {
      ...
      body: JSON.stringify({ env: resolveEbayEnv(process.env.EBAY_ENV), apiHost, ok: r.ok, status: r.status, body: json }),
    };
  } catch (e: any) {
    return { statusCode: 500, body: JSON.stringify({ error: e?.message || 'diag-privileges failed' }) };
  }
};
