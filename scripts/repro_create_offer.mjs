...
(async ()=>{
  ...
  const itemPayload = { condition: 'NEW', availability: { shipToLocationAvailability: { quantity: 1 } }, product: { title: 'Test', description: 'test', imageUrls: ['https://via.placeholder.com/600'] } };
  ...
  const fulfillment = await call('/sell/account/v1/fulfillment_policy?marketplace_id=EBAY_US','GET');

  const paymentId = (payment.body?.paymentPolicies && payment.body.paymentPolicies[0]?.paymentPolicyId) || process.env.EBAY_PAYMENT_POLICY_ID;
  const returnId = (returnP.body?.returnPolicies && returnP.body.returnPolicies[0]?.returnPolicyId) || process.env.EBAY_RETURN_POLICY_ID;
  const fulfillmentId = (fulfillment.body?.fulfillmentPolicies && fulfillment.body.fulfillmentPolicies[0]?.fulfillmentPolicyId) || process.env.EBAY_FULFILLMENT_POLICY_ID;
