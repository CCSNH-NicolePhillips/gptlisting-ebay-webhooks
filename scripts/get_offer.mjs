...
async function main(offerId){
  ...
  if(!clientId || !clientSecret){ console.error('Missing EBAY_CLIENT_ID/SECRET in env'); process.exit(4); }
...
if(!offerId){ console.error('Usage: node scripts/get_offer.mjs <offerId>'); process.exit(1); }
main(offerId).catch(e=>{ console.error('ERR', e); process.exit(1); });
