...
async function main() {
  ...
  const getInv = await fetch(
    `${base}/sell/inventory/v1/inventory_item/${encodeURIComponent(sku)}`,
    {
      method: 'GET',
      headers: {
  ...
  if (getInv.status !== 200) {
    console.error('Failed to get inventory', getInv.status, invText);
    process.exit(2);
  }
  ...
  if (!putInv.ok) {
    console.error(await putInv.text());
    process.exit(3);
  }

  console.log('Fetching offer', offerId);
  const getOffer = await fetch(`${base}/sell/inventory/v1/offer/${encodeURIComponent(offerId)}`, {
    method: 'GET',
    headers: {
      Authorization: `Bearer ${access}`,
      'Accept-Language': 'en-US',
  ...
  if (getOffer.status !== 200) {
    console.error('Failed to get offer', getOffer.status, offerText);
    process.exit(4);
  }
  ...
  const putOffer = await fetch(`${base}/sell/inventory/v1/offer/${encodeURIComponent(offerId)}`, {
    method: 'PUT',
    headers: {
      Authorization: `Bearer ${access}`,
      'Content-Type': 'application/json',
  ...
  if (!putOffer.ok) {
    console.error(putOfferText);
    process.exit(5);
  }

  console.log('Fetching updated offer');
  const final = await fetch(`${base}/sell/inventory/v1/offer/${encodeURIComponent(offerId)}`, {
    method: 'GET',
    headers: {
      Authorization: `Bearer ${access}`,
      'Accept-Language': 'en-US',
