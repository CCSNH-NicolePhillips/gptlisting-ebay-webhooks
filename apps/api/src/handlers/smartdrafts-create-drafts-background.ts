...
import { type PriceDecision } from "../../../../src/lib/pricing/index.js";
import { getPricingDecision, type PricingDecision, type DeliveredPricingSettings } from "../../../../src/lib/pricing/index.js";
...
function buildPrompt(
  ...
  lines.push("- 'Hallusa Hallosmine Ultra Diosmin Hesperidin Vein Circulation Support'");
  lines.push("- 'r.e.m. beauty Wicked Hydrating Undereye Masks Luxury Skincare Brightening'");
  ...
  lines.push("- 'Hallosmine Ultra 60 Capsules NEW' ← Missing brand, count/NEW wasted");
...
async function createDraftForProduct(
  ...
  try {
    ...
    priceResult = await getPricingDecision({
      brand: product.brand || '',
    ...
    if (isBundle && simpleBundleTitle && (!priceResult.pricingEvidence.summary?.canCompete || priceResult.finalItemCents < 1000)) {
      console.log(`[smartdrafts-price] 🎁 Full bundle query failed, trying simplified: "${product.brand} ${simpleBundleTitle}"`);
      
      const simplifiedResult = await getPricingDecision({
        brand: product.brand || '',
      ...
      if (simplifiedResult.pricingEvidence.summary?.canCompete && simplifiedResult.finalItemCents >= 1000) {
        console.log(`[smartdrafts-price] 🎁 Simplified query found SET price: $${(simplifiedResult.finalItemCents / 100).toFixed(2)}`);
        priceResult = simplifiedResult;
      }
    }
    
    // If bundle pricing still failed, try summing individual prices
    if (isBundle && (!priceResult.pricingEvidence.summary?.canCompete || priceResult.finalItemCents < 1000)) {
      ...
      for (const individualProduct of bundleProducts) {
        try {
          const individualResult = await getPricingDecision({
            brand: product.brand || '',
          ...
          if (individualResult.finalItemCents > 0) {
            totalCents += individualResult.finalItemCents;
            console.log(`[smartdrafts-price]   + ${individualProduct}: $${(individualResult.finalItemCents / 100).toFixed(2)}`);
          }
        } catch (e) {
      ...
      if (totalCents > priceResult.finalItemCents) {
        ...
        // Override with bundle pricing
        priceResult = {
          ...priceResult,
          finalItemCents: bundlePriceCents,
          warnings: [...priceResult.warnings, 'bundlePriceSummed'],
        };
      }
    }
    
    const _peSummary = priceResult.pricingEvidence.summary;
    console.log(`[smartdrafts-price] Result: item=$${(priceResult.finalItemCents / 100).toFixed(2)}, ship=$${(priceResult.finalShipCents / 100).toFixed(2)}, canCompete=${_peSummary?.canCompete}`);
    
    // Log detailed pricing info to Redis for debugging
    logger.info(`Pricing for ${product.brand} ${priceLookupTitle}`, {
      finalItemCents: priceResult.finalItemCents,
      finalShipCents: priceResult.finalShipCents,
      canCompete: _peSummary?.canCompete,
      compsSource: _peSummary?.compsSource,
      ebayFloor: _peSummary?.activeFloorDeliveredCents,
      soldMedian: _peSummary?.soldMedianDeliveredCents,
      soldCount: _peSummary?.soldCount,
      amazonPrice: _peSummary?.amazonPriceCents,
      walmartPrice: _peSummary?.walmartPriceCents,
      retailCompsCount: _peSummary?.retailCompsCount ?? 0,
      warnings: priceResult.warnings,
    });
    
    // Check for niche brand warning - this is a BLOCKING issue
    const hasNicheBrandWarning = priceResult.warnings?.includes('nicheBrandNeedsReview');
    ...
    const itemDollars = priceResult.finalItemCents / 100;
    
    const _topComps = _peSummary?.topComps ?? [];
    const _ebayTopComps = _topComps.filter(c => !['amazon', 'walmart', 'target'].includes(c.source));
    const _retailTopComps = _topComps.filter(c => ['amazon', 'walmart', 'target'].includes(c.source));

    if (_peSummary?.skipListing) {
    ...
    } else if (!_peSummary?.canCompete) {
      // Overpriced but not skipping (FLAG_ONLY mode)
      pricingDecision = {
        ok: true,
        source: _peSummary?.compsSource as any,
        price: itemDollars,
        recommendedListingPrice: itemDollars,
        candidates: _ebayTopComps.map(c => ({
        ...
        manualReviewReason: `Cannot compete: market floor $${((_peSummary?.activeFloorDeliveredCents ?? 0) / 100).toFixed(2)} delivered, our price $${((priceResult.finalItemCents + priceResult.finalShipCents) / 100).toFixed(2)}`,
      };
    } else {
      ...
      pricingDecision = {
        ...
        source: _peSummary?.compsSource as any,
        price: itemDollars,
        confidence: _peSummary?.matchConfidence,
        recommendedListingPrice: itemDollars,
        candidates: [
          ..._ebayTopComps.slice(0, 3).map(c => ({
            source: 'ebay-sold' as const,
            price: c.deliveredCents / 100,
            url: c.url || undefined,
            currency: 'USD',
          })),
          ..._retailTopComps.slice(0, 2).map(c => ({
        ...
        needsManualReview: _peSummary?.matchConfidence === 'low',
        manualReviewReason: _peSummary?.matchConfidence === 'low' 
    ...
    if (!pricingDecision) {
      ...
      priceMeta = {
        ...
        ...(priceResult?.pricingEvidence.summary && {
          canCompete: priceResult.pricingEvidence.summary.canCompete,
          compsSource: priceResult.pricingEvidence.summary.compsSource,
          ebayCompsCount: priceResult.pricingEvidence.ebayCompsCount,
          retailCompsCount: priceResult.pricingEvidence.summary.retailCompsCount,
          shippingEstimateSource: priceResult.pricingEvidence.summary.shippingEstimateSource,
        }),
      };
    } else {
      finalPrice = pricingDecision.recommendedListingPrice;
      ...
      priceMeta = {
        ...
        ...(priceResult?.pricingEvidence.summary && {
          ...
          ebayCompsCount: priceResult.pricingEvidence.ebayCompsCount,
          retailCompsCount: priceResult.pricingEvidence.summary.retailCompsCount,
          activeFloorCents: priceResult.pricingEvidence.summary.activeFloorDeliveredCents,
          finalItemCents: priceResult.finalItemCents,
          finalShipCents: priceResult.finalShipCents,
          freeShipApplied: priceResult.pricingEvidence.summary.freeShipApplied,
          shippingEstimateSource: priceResult.pricingEvidence.summary.shippingEstimateSource,
        }),
      };
  ...
  if (isBundleProduct) {
    const bundleCount = product.bundleInfo!.bundleProducts.length;
    const bundleType = product.bundleInfo!.bundleType || 'set';
    const wasSummed = priceResult?.warnings?.includes('bundlePriceSummed');
  ...
  if (resolvedPrice !== null && priceResult?.pricingEvidence.summary) {
    const retailPrices = [
      priceResult.pricingEvidence.summary.amazonPriceCents,
      priceResult.pricingEvidence.summary.walmartPriceCents,
  ...
  try {
    ...
    const _logEbayComps = priceResult?.pricingEvidence.summary?.topComps?.filter(
      c => !['amazon', 'walmart', 'target'].includes(c.source)
    ) ?? [];
    if (_logEbayComps.length > 0) {
      pricingSourceLogs.push({
        source: 'eBay Active',
        query: `${product.brand || ''} ${product.product}`.trim(),
        results: _logEbayComps.slice(0, 5).map(c => ({
          title: c.title || 'eBay Listing',
          price: c.itemCents / 100,
          shipping: c.shipCents / 100,
          total: c.deliveredCents / 100,
          url: c.url || undefined,
          seller: c.seller,
        })),
        selectedResult: priceResult?.pricingEvidence.summary?.activeFloorDeliveredCents ? {
          title: 'Lowest eBay Active',
          price: priceResult.pricingEvidence.summary.activeFloorDeliveredCents / 100,
    ...
    if (_logAmazon || _logWalmart) {
      pricingSourceLogs.push({
        ...
        results: [
          ...
          ...(_logWalmart ? [{
            title: 'Walmart',
            price: _logWalmart / 100,
            shipping: 0,
            total: _logWalmart / 100,
          }] : []),
        ],
      });
    }
    
    // Build calculations from delivered evidence
    const _logEv = priceResult?.pricingEvidence;
    const _logSmr = _logEv?.summary;
    const _logEbayForCalc = _logSmr?.topComps?.filter(c => !['amazon', 'walmart', 'target'].includes(c.source)) ?? [];
    const _logRetailForCalc = _logSmr?.topComps?.filter(c => ['amazon', 'walmart', 'target'].includes(c.source)) ?? [];
    const calculations = priceResult ? buildPricingCalculations(
      {
        targetDeliveredCents: _logEv?.targetDeliveredCents,
        finalItemCents: priceResult.finalItemCents,
        finalShipCents: priceResult.finalShipCents,
        freeShipApplied: _logSmr?.freeShipApplied,
        subsidyCents: _logSmr?.subsidyCents,
        shippingEstimateCents: pricingSettings.templateShippingEstimateCents || 600,
        ebayComps: _logEbayForCalc,
        retailComps: _logRetailForCalc,
        activeFloorDeliveredCents: _logSmr?.activeFloorDeliveredCents,
        amazonPriceCents: _logSmr?.amazonPriceCents,
        walmartPriceCents: _logSmr?.walmartPriceCents,
        soldMedianDeliveredCents: _logSmr?.soldMedianDeliveredCents,
    ...
    if (_logSmr?.canCompete && (_logSmr.soldCount || 0) >= 3) {
      confidence = 'high';
    } else if (_logSmr?.canCompete || (_logSmr?.soldCount || 0) >= 1) {
      confidence = 'medium';
    }
    ...
    await storeDraftLogs(userId, product.productId, {
      ...
      pricing: {
        ...
        warnings: priceResult?.warnings || [],
        competitorSummary: {
          lowestDeliveredPrice: _logSmr?.activeFloorDeliveredCents 
            ? _logSmr.activeFloorDeliveredCents / 100 
            : undefined,
          medianDeliveredPrice: _logSmr?.soldMedianDeliveredCents 
            ? _logSmr.soldMedianDeliveredCents / 100 
            : undefined,
          amazonPrice: _logSmr?.amazonPriceCents 
            ? _logSmr.amazonPriceCents / 100 
            : undefined,
          walmartPrice: _logSmr?.walmartPriceCents 
            ? _logSmr.walmartPriceCents / 100 
            : undefined,
