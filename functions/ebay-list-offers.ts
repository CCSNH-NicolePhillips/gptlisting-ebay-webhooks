...
export const handler: Handler = async (event) => {
  ...
  const saved = (await store.get(userScopedKey(sub, 'ebay.json'), { type: 'json' })) as any;
    ...
    // Helper: normalize status filter (supports CSV list)
    const normalizeStatuses = (s?: string | null) => {
    ...
    async function listOnce(includeStatus: boolean, includeMarketplace: boolean) {
      ...
      if (includeStatus && requestedStatuses.length)
        params.set('offer_status', requestedStatuses.join(','));
      ...
      for (let pages = 0; pages < 10; pages++) {
        ...
        for (const it of items) {
          ...
          for (const o of arr) {
            ...
            if (!requestedStatuses.length || requestedStatuses.includes(st)) agg.push(o);
          }
    ...
    const final = requestedStatuses.length
      ? offers.filter((o: any) => requestedStatuses.includes(String(o?.status || '').toUpperCase()))
      : offers;
