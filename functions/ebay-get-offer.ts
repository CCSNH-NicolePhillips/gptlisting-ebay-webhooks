...
export const handler: Handler = async (event) => {
  try {
    ...
    if (!r.ok) return { statusCode: r.status, body: JSON.stringify({ error: "get-offer failed", url, status: r.status, detail: json }) };
    return { statusCode: 200, headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ok: true, offer: json }) };
  } catch (e: any) {
    return { statusCode: 500, body: JSON.stringify({ error: "get-offer error", detail: e?.message || String(e) }) };
  }
};
