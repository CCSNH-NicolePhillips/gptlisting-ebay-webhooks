  ...
  try {
    ...
    if (!cached) {
      ...
      function flatten(node: any, path: string[] = []): any[] {
        if (!node) return [];
        const name = node.categoryTreeNode?.category?.categoryName || node.category?.categoryName || node.categoryName;
        const id = node.categoryTreeNode?.category?.categoryId || node.category?.categoryId || node.categoryId;
        const children = node.children || node.childCategoryTreeNodes || [];
        const here = id && name ? [{ categoryId: id, categoryName: name, categoryPath: [...path, name].join(' > ') }] : [];
    ...
    if (q) list = list.filter(c => (c.categoryName||'').toLowerCase().includes(q) || (c.categoryPath||'').toLowerCase().includes(q));

    // Limit results for UI performance
    const limit = Number(event.queryStringParameters?.limit || 200);
    return { statusCode: 200, headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ok: true, treeId: cached.treeId, count: cached.count, results: list.slice(0, limit) }) };
  } catch (e: any) {
    return { statusCode: 500, body: JSON.stringify({ error: 'category-tree error', detail: e?.message || String(e) }) };
  }
};
