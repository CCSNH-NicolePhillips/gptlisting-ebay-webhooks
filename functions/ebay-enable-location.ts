...
export const handler: Handler = async (event) => {
  try {
    ...
    if (!refresh) return { statusCode: 400, headers: { "Content-Type": "application/json" }, body: JSON.stringify({ error: "Connect eBay first" }) };
    ...
    return { statusCode: 500, headers: { "Content-Type": "application/json" }, body: JSON.stringify({ error: "enable-location error", detail: e?.message || String(e) }) };
  }
};
