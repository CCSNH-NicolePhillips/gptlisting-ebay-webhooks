...
export const handler: Handler = async () => {
  try {
    ...
    if (!refresh) return { statusCode: 400, headers: { "Content-Type": "application/json" }, body: JSON.stringify({ error: "missing-refresh-token", hint: "Connect eBay or set EBAY_TEST_REFRESH_TOKEN" }) };
    ...
    return { statusCode: 500, headers: { "Content-Type": "application/json" }, body: JSON.stringify({ error: "list-locations error", detail: e?.message || String(e) }) };
  }
};
