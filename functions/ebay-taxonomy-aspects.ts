...
export const handler: Handler = async (event) => {
  try {
    ...
    if (!categoryId)
      return { statusCode: 400, body: JSON.stringify({ error: 'Missing categoryId' }) };
    ...
    async function getTreeId(): Promise<string> {
      ...
      const tid = j?.categoryTreeId;
      if (!tid) throw new Error('No treeId');
      return String(tid);
    }
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        treeId,
        categoryId,
        required: aspects.filter((x: any) => x.required),
        optional: aspects.filter((x: any) => !x.required),
      }),
    };
  } catch (e: any) {
    return {
      statusCode: 500,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ error: 'taxonomy-aspects error', detail: e?.message || String(e) }),
    };
  }
};
