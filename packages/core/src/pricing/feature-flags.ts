...
export interface PricingFeatureFlags {
   ...
   *  @deprecated `getDeliveredPricing()` now always runs the v2 pipeline.
   *  This flag no longer gates the v2/legacy split inside delivered-pricing.ts.
   *  Use `PRICING_MODE=delivered_v2 | legacy` (via src/lib/pricing/index.ts) instead.
   *  Kept in interface for backward source compatibility only. */
  v2Enabled: boolean;
 ...
 * - DP_PRICING_V2          → v2Enabled   (@deprecated — no longer controls routing)
